//! FreeBSD and NetBSD PTY integration tests.
//!
//! The test workflow is shared, but target-specific `speed_t` fields preserve
//! each native `termios` layout. `openpty` supplies the real pseudoterminal used
//! to observe raw mode, resize polling, UTF-8 input, and restoration.
//!
//! References:
//! - FreeBSD src [`_termios.h`](https://cgit.freebsd.org/src/tree/sys/sys/_termios.h),
//!   [`ttycom.h`](https://cgit.freebsd.org/src/tree/sys/sys/ttycom.h), and
//!   [`fcntl.h`](https://cgit.freebsd.org/src/tree/sys/sys/fcntl.h)
//! - NetBSD src [`termios.h`](https://nxr.netbsd.org/xref/src/sys/sys/termios.h),
//!   [`ttycom.h`](https://nxr.netbsd.org/xref/src/sys/sys/ttycom.h), and
//!   [`fcntl.h`](https://nxr.netbsd.org/xref/src/sys/sys/fcntl.h)
...
// fcntl values and ioctl request shared by the two referenced ABIs.
...
const _: () = {
    assert!(size_of::<BsdTermios>() == 44);
    assert!(align_of::<BsdTermios>() == 4);
    assert!(size_of::<WindowSize>() == 8);
    assert!(align_of::<WindowSize>() == 2);
};

// Both systems provide openpty through libutil.
#[link(name = "util")]
unsafe extern "C" {
    fn openpty(
        master: *mut c_int,
        slave: *mut c_int,
        name: *mut c_char,
        termios: *const BsdTermios,
        size: *const WindowSize,
    ) -> c_int;
}

// Remaining descriptor functions come from the system C ABI.
unsafe extern "C" {
    fn tcgetattr(file_descriptor: c_int, termios: *mut BsdTermios) -> c_int;
...
impl PseudoTerminal {
    fn new(rows: u16, columns: u16) -> io::Result<Self> {
        ...
        // termios pointers are null, and `size` has the target `winsize` layout.
    ...
    fn resize(&mut self, rows: u16, columns: u16) -> io::Result<()> {
        ...
        // SAFETY: the slave fd is live and `size` has the target `winsize` layout.
        check_not_minus_one(unsafe { ioctl(self.slave.as_raw_fd(), TIOCSWINSZ, &size) }).map(|_| ())
    }

    fn termios(&self) -> io::Result<BsdTermios> {
        let mut termios = std::mem::MaybeUninit::<BsdTermios>::uninit();
        // SAFETY: the slave fd is live and `termios` is writable storage with
        // the selected target's exact C layout.
