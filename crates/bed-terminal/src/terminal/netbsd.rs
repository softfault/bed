//! NetBSD terminal backend.
//!
//! This module uses the NetBSD C `termios` and `ioctl` ABI directly. It is
//! limited to x86_64 because that is the target currently cross-checked by bed.
//! NetBSD stores the speed fields as C `int`, unlike FreeBSD's `speed_t` fields.
//!
//! Authoritative references:
//! - NetBSD src [`termios.h`](https://nxr.netbsd.org/xref/src/sys/sys/termios.h)
//!   and [`ttycom.h`](https://nxr.netbsd.org/xref/src/sys/sys/ttycom.h)
//! - NetBSD man pages [`termios(4)`](https://man.netbsd.org/termios.4)
...
// Raw-mode flags and control-character indices from NetBSD termios.h.
...
/// NetBSD `struct winsize` from ttycom.h.
#[repr(C)]
struct WindowSize {
    rows: u16,
    columns: u16,
...
const _: () = {
    // Reject a target at compile time if its C layout differs from the NetBSD
    // ABI selected by this module.
    assert!(size_of::<NetBsdTermios>() == 44);
    assert!(align_of::<NetBsdTermios>() == 4);
    assert!(size_of::<WindowSize>() == 8);
    assert!(align_of::<WindowSize>() == 2);
};

// Function signatures follow NetBSD termios.h and <sys/ioctl.h>.
unsafe extern "C" {
    fn tcgetattr(file_descriptor: c_int, termios: *mut NetBsdTermios) -> c_int;
    fn tcsetattr(
        file_descriptor: c_int,
        optional_actions: c_int,
        termios: *const NetBsdTermios,
    ) -> c_int;
    fn ioctl(file_descriptor: c_int, request: c_ulong, ...) -> c_int;
}

pub(super) struct PlatformTerminal {
    original: NetBsdTermios,
    input_fd: c_int,
    output_fd: c_int,
...
fn get_termios(file_descriptor: c_int) -> Result<NetBsdTermios> {
    let mut termios = MaybeUninit::<NetBsdTermios>::uninit();
    // SAFETY: `file_descriptor` belongs to the live stdin handle, and the
    // output pointer is valid for a NetBSD `struct termios`.
...
fn set_termios(file_descriptor: c_int, termios: &NetBsdTermios) -> Result<()> {
    // SAFETY: the descriptor is live and `termios` has NetBSD's exact C layout.
...
fn get_window_size(file_descriptor: c_int) -> Result<TerminalSize> {
    ...
    // is writable storage for NetBSD `struct winsize`.
