...
Generated terminal responses remain pending while that bounded input channel
is full and are retried before additional child output is consumed.
...
without discarding the session. It also verifies child-controlled cursor
visibility and clears view-local selection or scrollback when the child changes
...
6. Enter character Visual mode with `v` and line Visual mode with `V`. Exercise motions, counts, reverse selection, Unicode graphemes, `y`, `d`/`x`, `p`/`P`, and `<`/`>`. Confirm shifting keeps the selection active, uses one undo step, and removes either one tab or at most four leading spaces when shifting left. Confirm `:s` affects selected lines without displaying Vim range markers, while explicit `:%s` still affects the complete buffer. An empty register must leave the selection active. In Normal mode, verify `<<`, `>>`, and counted shifts operate on the expected consecutive lines.
...
11. Exercise the file tree with nested directories and non-ASCII names. Verify that its header follows the current root directory name, use `Ctrl-W h/l/w` to move between it and the editor, navigate upward with `..`, use `H` and `L` to move the root in both directions, adjust and reset its width, set an exact `:treewidth`, expand, collapse, refresh, change its root with `:tree`, and open a file into the active window. From an embedded terminal and a second process, create, rename, and delete files in the root and an expanded directory; each change must appear without pressing `r`. Generate several thousand files and confirm that input and terminal output remain responsive while the tree updates. Give two tabs different roots and selections; switching tabs must restore each tree while retaining one global panel width.
12. Run `:! echo ready` and confirm it opens a terminal split, retains `ready` after the command exits, and leaves the editor responsive.
13. Modify both buffers. Verify that hidden changes block `:q`, then save them with `:wall`. Change a clean open file from another process and confirm it reloads automatically. Change a dirty open file externally and confirm the local text remains with `[conflict]`; delete an open file and confirm its content remains with `[deleted]`. Normal writes must refuse conflicts, while `:w!` resolves them. Repeat an external save using temporary-file-plus-rename replacement.
14. Resize the terminal while bed is idle; all windows redraw without another key press, and the file tree disappears below its narrow-terminal threshold.
15. Confirm Normal and Visual modes use a block cursor, Insert mode uses a bar cursor, and command/search/tree modes return to a block cursor.
16. Exit with `:q`; each saved file contains the exact UTF-8 text and line structure entered above.
17. Confirm that the original screen, cursor visibility, default cursor shape, bracketed-paste mode, input echo, and canonical line input are restored.
